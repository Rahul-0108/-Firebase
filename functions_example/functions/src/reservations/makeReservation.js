...
// onCall : We can call this function directly as onCall(data) from frontend instead of making http Request
export const makeReservation = functions.https.onCall(async (data, context) => {
  ...
  const requestedDateDoc = await store.collection("dateAvailabilities").doc(availabilityId).get();
  ...
  await store.collection("reservations").add({
    userId,
    createdAt: Date.now(),
